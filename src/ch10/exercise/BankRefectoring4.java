...
//List대신에 Key로 검색을 쉽게 할 수 있도록, 자료구조를 변경하라.
//Map<String, Account> = Map<accountNo, Account>
public class BankRefectoring4 {
...
//		ArrayList<Account> accounts = new ArrayList<>();
		// 인터페이스를 Map 데이터타입으로 줌 -> HashMap이나 다른 것들로 사용하기 위해
...
//				withrawAccount(in, accounts);
	...
	private static void createAccount(Scanner in, Map<String, Account> accounts) {
		...
		accounts.put(accountNo, new Account(accountNo, accountOwner, balance, password));
//		accounts.add(new Account(accountNo, accountOwner, balance, password));
	...
	private static void listAccounts(Map<String, Account> accounts) {
		...
		// V가 매개변수가 여러 개인 객체일 경우, Entry로 K와 V의 매개변수 각각 사용할 수 있음.
//		for (Map.Entry<String, Account> entry : accounts.entrySet()) {
//			String key = entry.getKey();
//			Account value = entry.getValue();
//
//			System.out.println(accountNo + "\t" + value.getAccountOwner() + "\t" + value.getBalance());
//		}
...
//	}
		// 위로 인해 그냥 찍어도 accountNo, accountOwner, balance만 나옴
		for (Account account : accounts.values()) {
			System.out.println(account);
		}

//		for (Account a : accounts) {
//			System.out.println(a.getAccountNo() + "\t" + a.getAccountOwner() + "\t" + a.getBalance());
//
//		}
		return;
	}

	private static void depositAccounts(Scanner in, Map<String, Account> accounts) {
		Long y;
...
//	private static void withrawAccount(Scanner in, Map<String, Account> accounts) {
//		Long money;
